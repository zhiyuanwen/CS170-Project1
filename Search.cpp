#include <iostream>
#include <vector>
#include <queue>
#include <chrono>
#include <cmath>

using namespace std;
using namespace std::chrono;

// Global variable to avoid declaring multiple times
vector<vector<int>> goalState = {{1, 2, 3}, {4, 5, 6}, {7, 8, 0}};   // End 3x3 goal state to compare to, change for other puzzle versions

// Nodes to contain important info of puzzle state
struct TreeNode
{
    int gVal;                                   // Cost of traveling form one node to another
    int hVal;                                   // Heuristic value
    vector <vector <int>> currPuzzle;           // 2D vector puzzle state
    vector <TreeNode*> puzzleDeviation;         // All possible states the current puzzle can go to in a vector
};

// To use a priority queue properly, pair it's g(n) + h(n) along with the node it belongs to
struct QueuePair
{
    int combineVal;                             // Calculation of g(n) + h(n)
    TreeNode* selfNode;                         // The node that contains the info
    
    QueuePair(int combine, TreeNode* node) : combineVal(combine), selfNode(node) {}
};

// Since I want to priority queue based on the g(n) + h(n) value, then we overload the queue comparison to only consider those values
// I also want the reverse operator to order the queue from small to big instead of big to small
struct QueueOrdered
{
    bool operator()(const QueuePair& q1, const QueuePair& q2)
    {
        return q1.combineVal >= q2.combineVal;
    }
};

// Helper function to print vector, non-formatted.
void printVec(vector <vector <int>> outputVec)
{
    for(int a = 0; a < outputVec.size(); a++)
    {
        for(int b = 0; b < outputVec[0].size(); b++)
        {
            cout << outputVec[a][b] << " ";
        }
        cout << endl;
    }
}

// The amount of misplaced tiles compared to the goal state
int misplacedTiles(vector<vector<int>> puzzleNode)
{
    int numMisplace = 0;
    for(int i = 0; i < puzzleNode.size(); i++)
    {
        for(int j = 0; j < puzzleNode[0].size(); j++)
        {
            if(puzzleNode[i][j] != goalState[i][j])
            {
                numMisplace++;
            }
        }
    }
    return numMisplace;
}

// The manhattan distance for all numbers of the vector
int manhattanDistance(vector<vector<int>> puzzleNode)
{
    int numDistance = 0;
    int rowGoal = 0;
    int colGoal = 0;
    int totalNum = (puzzleNode.size() * puzzleNode[0].size()) - 1;
    
    for(int i = 0; i < totalNum; i++)
    {
        bool found = false;
        for(int j = 0; j < puzzleNode.size(); j++)
        {
            for(int k = 0; k < puzzleNode[0].size(); k++)
            {
                if(puzzleNode[j][k] == i + 1)
                {
                    numDistance += abs(j - rowGoal) + abs(k - colGoal);
                    found = true;
                    break;
                }
            }
            if(found)
            {
                colGoal++;
                if(colGoal == puzzleNode[0].size())
                {
                    rowGoal++;
                    colGoal = 0;
                }
                break;
            }
        }
    }
    
    return numDistance;
}

//Helper function to compare 2 vectors, both should be same size
//If a difference is found, they are not the same, otherwise they are the same state
bool compareVecs(vector <vector<int>> vecOne, vector <vector<int>> vecTwo)
{
    for(int a = 0; a < vecOne.size(); a++)
    {
        for(int b = 0; b < vecOne[0].size(); b++)
        {
            if(vecOne[a][b] != vecTwo[a][b])
            {
                return false;
            }
        }
    }

    return true;
}

// Helper function to trace back solution from root to goal state
bool findPath(TreeNode* rootNode, vector<TreeNode*>& traceVec)
{
    vector<vector<int>> puzzleVec = rootNode->currPuzzle;
    if(compareVecs(puzzleVec, goalState))
    {
        return true;
    }

    traceVec.push_back(rootNode);

    bool childGoal = false;
    for(int i = 0; i < rootNode->puzzleDeviation.size(); ++i)
    {
        if(findPath(rootNode->puzzleDeviation[i], traceVec))
        {
            childGoal = true;
            break;
        }
    }
    if(childGoal)
    {
        return true;
    }

    traceVec.pop_back();
    return false;
}

// Helper function to split the current node into the others
vector<vector<vector<int>>> allPossibles(TreeNode* originalNode)
{
    vector<vector<vector<int>>> allChildNodes;            // All possibilites or children from original node passed
    bool leftMove, rightMove, upMove, downMove = true;    // Set all possible moves to true
    vector<vector<int>> puzzleNow = originalNode->currPuzzle;
    int zeroPosRow;
    int zeroPosCol;
    
    // Find position of 0 in puzzle, keep track of which row and column it's in
    for(int i = 0; i < puzzleNow.size(); i++)
    {
        bool zeroFound = false;
        for(int j = 0; j < puzzleNow[0].size(); j++)
        {
            if(puzzleNow[i][j] == 0)
            {
                zeroFound = true;
                zeroPosRow = i;
                zeroPosCol = j;
                break;
            }
        }
        if(zeroFound)
        {
            break;
        }
    }
    
    if(zeroPosCol == 0)                                           // If leftmost column, left move not possible
    {
        leftMove = false;
    }
    else
    {
        leftMove = true;
    }
    if(zeroPosCol == originalNode->currPuzzle.size() - 1)    // Rightmost column means no right move
    {
        rightMove = false;
    }
    else
    {
        rightMove = true;
    }
    if(zeroPosRow == 0)                                           // Top row means no up move possible
    {
        upMove = false;
    }
    else
    {
        upMove = true;
    }
    if(zeroPosRow == originalNode->currPuzzle.size() - 1)    // Bottom row means no down move possible
    {
        downMove = false;
    }
    else
    {
        downMove = true;
    }

    // Make puzzle vector and int to swap values
    vector<vector<int>> swappedVec;
    int tempSwapVal = 0;
    // For each of the 4 possible moves of the blank square (0), swap elements in that move and store in possible end puzzles
    if(leftMove)
    {
        swappedVec = originalNode->currPuzzle;
        tempSwapVal = originalNode->currPuzzle[zeroPosRow][zeroPosCol - 1];
        swappedVec[zeroPosRow][zeroPosCol - 1] = 0;
        swappedVec[zeroPosRow][zeroPosCol] = tempSwapVal;

        allChildNodes.push_back(swappedVec);
    }
    if(rightMove)
    {
        swappedVec = originalNode->currPuzzle;
        tempSwapVal = originalNode->currPuzzle[zeroPosRow][zeroPosCol + 1];
        swappedVec[zeroPosRow][zeroPosCol + 1] = 0;
        swappedVec[zeroPosRow][zeroPosCol] = tempSwapVal;

        allChildNodes.push_back(swappedVec);
    }
    if(upMove)
    {
        swappedVec = originalNode->currPuzzle;
        tempSwapVal = originalNode->currPuzzle[zeroPosRow - 1][zeroPosCol];
        swappedVec[zeroPosRow - 1][zeroPosCol] = 0;
        swappedVec[zeroPosRow][zeroPosCol] = tempSwapVal;

        allChildNodes.push_back(swappedVec);
    }
    if(downMove)
    {
        swappedVec = originalNode->currPuzzle;
        tempSwapVal = originalNode->currPuzzle[zeroPosRow + 1][zeroPosCol];
        swappedVec[zeroPosRow + 1][zeroPosCol] = 0;
        swappedVec[zeroPosRow][zeroPosCol] = tempSwapVal;

        allChildNodes.push_back(swappedVec);
    }
    return allChildNodes;
}

// The search algorithm
void SearchAlgorithm(vector <vector <int>> puzzleVec, int funcNum)
{
    if(compareVecs(puzzleVec, goalState))
    {
        cout << "You entered the goal state" << endl;
        return;
    }
    TreeNode* rootNode = new TreeNode;                         // Starting position, KEPT AS IS
    rootNode->gVal = 0;                                        // Root node is g(n) = 0 since no moves are made
    rootNode->currPuzzle = puzzleVec;                          // Set puzzle to the root node
    // h(n) is dependent on uniform(1) or A* with misplace(2) or A* with manhattan(3)
    if(funcNum == 1)
    {
        rootNode->hVal = 0;
    }
    else if(funcNum == 2)
    {
        rootNode->hVal = misplacedTiles(puzzleVec);
    }
    else if(funcNum == 3)
    {
        rootNode->hVal = manhattanDistance(puzzleVec);
    } 

    int totalNum = 0;                                                        // Total number of nodes searched
    vector<vector<vector<int>>> repeatStates;                                // Vector of all possible 2D states from starting puzzle
    repeatStates.push_back(puzzleVec);                                       // It starts with initial puzzle
    priority_queue <QueuePair, vector<QueuePair>, QueueOrdered> nodes;      // Queue of states
    
    QueuePair pairNew(rootNode->gVal + rootNode->hVal, rootNode);
    nodes.push(pairNew);

    while(nodes.size() > 0)
    {
        // Get the first node in the queue
        QueuePair currPair = nodes.top();
        
        TreeNode* currNode = new TreeNode;
        currNode = currPair.selfNode;
        nodes.pop();
        
        // Get all possible next states of the vector, remove any states that are already encountered (stored in repeatStates), add amount to total nodes searched
        vector<vector<vector<int>>> poStates = allPossibles(currNode);
        
        for(int i = 0; i < poStates.size(); i++)
        {
            for(int j = 0; j < repeatStates.size(); j++)
            {
                if(compareVecs(poStates[i], repeatStates[j]))
                {
                    poStates.erase(poStates.begin() + i);
                    i--;
                    break;
                }
            }
        }
        totalNum += poStates.size();
        
        // Check if any states are the goal state, otherwise add to repeatStates and queue, create nodes to tree, repeat
        bool finished = false;
        vector<TreeNode*> currChildren;
        for(int i = 0; i < poStates.size(); i++)
        {
            TreeNode* tempNode = new TreeNode;
            
            tempNode->gVal = currNode->gVal + 1;       // All children are 1 increased value from its parent g(n) since that is a move
            tempNode->currPuzzle = poStates[i];
            // h(n) changes based on which search is used
            if(funcNum == 1)
            {
                tempNode->hVal = 0;
            }
            else if(funcNum == 2)
            {
                tempNode->hVal = misplacedTiles(tempNode->currPuzzle);
            }
            else if(funcNum == 3)
            {
                tempNode->hVal = manhattanDistance(tempNode->currPuzzle);
            }
            
            repeatStates.push_back(poStates[i]);
            currChildren.push_back(tempNode);
            
            QueuePair newPair(tempNode->gVal + tempNode->hVal, tempNode);
            nodes.push(newPair);
            
            if(compareVecs(poStates[i], goalState))
            {
                finished = true;
                break;
            }
        }
        currNode->puzzleDeviation = currChildren;
        if(finished) // Break out of while loop since goal state was reached
        {
            break;
        }
        cout << "Running " << totalNum << endl;
    }
    if(nodes.size() == 0)
    {
        cout << "Solution not found, invalid puzzle, test" << endl;
    }
    else
    {
        vector<TreeNode*> solutionTrace;
        findPath(rootNode, solutionTrace);
        for(int i = 0; i < solutionTrace.size(); ++i)
        {
            cout << "Best node to expand with g(n) = " << solutionTrace[i]->gVal << " and h(n) = " << solutionTrace[i]->hVal << endl;
            printVec(solutionTrace[i]->currPuzzle);
            cout << endl;
        }
        cout << "Goal state found after " << totalNum << " nodes expanded" << endl;
        cout << "The solution is depth "  << solutionTrace.size() << endl;
    }
}

int main()
{
    vector <vector <int>> userPuzzle;                          // Current state of puzzle
    //userPuzzle = {{1, 2, 3}, {4, 5, 6}, {0, 7, 8}};
    
    // Input calls for 3x3 puzzle
    int tempVal;
    for(int i = 0; i < 3; i++)
    {
        cout << "Enter numbers for row " << i + 1 << ": "; 
        vector <int> tempVec;
        for(int j = 0; j < 3; j++)
        {
            cin >> tempVal;
            tempVec.push_back(tempVal);
        }
        userPuzzle.push_back(tempVec);
    }

    // Input call for which method to use
    do
    {
        cout << "Select algorithm. (1) for Uniform Cost Search, (2) for Misplaced Tile Heuristic, (3) for Manhattan Distance Heuristic. ";
        cin >> tempVal;
    } while (tempVal < 1 || tempVal > 3);
    cout << endl;

    auto start = high_resolution_clock::now();
    SearchAlgorithm(userPuzzle, tempVal);
    auto end = high_resolution_clock::now();
    
    auto duration = duration_cast<microseconds>(end - start);
    cout << "Total time taken is " << duration.count() << " ms" << endl;
    
    return 0;
}